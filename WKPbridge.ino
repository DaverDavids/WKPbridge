// WKPbridge.ino
// Minimal ESP32-C3 BLE bridge + web UI + OTA + captive portal fallback
...
struct Settings {
  ...
  String bleAddress;
  String bleName;       // optional substring match during scan
  String serviceUuid;
  String writeUuid;
  String notifyUuid;
...
void ensureMDNSOTA() {
  ...
  if (!otaReady) {
    ...
    ArduinoOTA.onError([](ota_error_t error) {
      addLog("OTA error " + String((int)error));
    });
    ArduinoOTA.begin();
    otaReady = true;
    addLog("OTA ready");
...
class ClientCB : public NimBLEClientCallbacks {
  void onConnect(NimBLEClient *pClient) override {
    addLog("BLE connected: " + String(pClient->getPeerAddress().toString().c_str()));
  }
...
bool connectByAddressString(const String &target) {
  std::string s(target.c_str());

  bleClient = makeClient();
  if (bleClient->connect(NimBLEAddress(s, BLE_ADDR_PUBLIC))) {
    addLog("BLE connected using PUBLIC address type");
    return true;
  }

  deleteBleClient();
  bleClient = makeClient();
  if (bleClient->connect(NimBLEAddress(s, BLE_ADDR_RANDOM))) {
    addLog("BLE connected using RANDOM address type");
    return true;
  }
...
bool connectBle() {
  ...
  if (settings.serviceUuid.isEmpty() || settings.writeUuid.isEmpty()) {
    addLog("BLE connect skipped: service/write UUID missing");
    return false;
  }

  addLog("BLE connect to " + target);
  if (!connectByAddressString(target)) {
  ...
  if (!writeChr) {
    addLog("BLE write characteristic not found");
    deleteBleClient();
    return false;
  }
  ...
  if (settings.notifyUuid.length()) {
    ...
    if (notifyChr && (notifyChr->canNotify() || notifyChr->canIndicate())) {
      ...
      addLog("Notify characteristic missing or not subscribable");
    }
  }
...
String makeStateJson() {
  ...
  String blePeer = (bleClient && bleClient->isConnected()) ? String(bleClient->getPeerAddress().toString().c_str()) : "";
  ...
  json += "\"ffHex\":\"DE C0 AD DE FF 01 1E F1\",";
  json += "\"feHex\":\"DE C0 AD DE FE 01 1E F1\",";
...
void sendOk(const String &msg) {
  server.send(200, "application/json", "{\"ok\":true,\"msg\":\"" + jsonEscape(msg) + "\"}");
}

void sendErr(const String &msg) {
  server.send(200, "application/json", "{\"ok\":false,\"msg\":\"" + jsonEscape(msg) + "\"}");
}
...
void handleConnect() {
  bool ok = connectBle();
  if (ok) sendOk("BLE connected");
  else sendErr("BLE connect failed");
}

void handleDisconnect() {
  disconnectBle();
  sendOk("BLE disconnected");
}

void handleSendFF() {
  bool ok = sendPacket(PKT_FF, sizeof(PKT_FF), "FF");
  if (ok) sendOk("FF sent");
  else sendErr("FF send failed");
}

void handleSendFE() {
  bool ok = sendPacket(PKT_FE, sizeof(PKT_FE), "FE");
  if (ok) sendOk("FE sent");
  else sendErr("FE send failed");
}

void handleSendRaw() {
  bool ok = sendRawHex(server.arg("hex"));
  if (ok) sendOk("Raw sent");
  else sendErr("Raw send failed");
}
...
void serviceWiFi() {
  ...
  if (!connected) {
    ...
    if (!portalMode && millis() > 30000 && millis() - lastWifiTryMs > 10000) {
      startPortal();
    }
  }
}
...
void setup() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    addLog("WiFi connect timeout, starting captive portal");
    startPortal();
  } else {
    ensureMDNSOTA();
  }
...
void loop() {
  ...
  if (millis() - lastStateMs > 10000) {
    lastStateMs = millis();
    addLog(
      "State wifi=" + String(WiFi.status() == WL_CONNECTED ? "up" : "down") +
